...
```
```{r}
ggplot(df, aes(Cond, PosAff, fill = Cond)) +
  geom_violin() +
  geom_boxplot(width = 0.1, color = "black", alpha = 0.4) +
  scale_fill_viridis(discrete=TRUE, name = "Condition")+
...
#### Fitting our model
```{r}
get_prior(mvbind(ANS_Z, ANF_Z) ~ Cond + PosAff_Z + pKM_Z + Time + Cond*Time (1|p|id),
    data = df_L)
```

```{r}
get_prior(mvbind(ANS_Z, ANF_Z) ~ Cond*Time + PosAff_Z + pKM_Z + (1|p|id),
    data = df_L)
```

```{r}
fit_A1 <- brm(mvbind(ANS_Z, ANF_Z) ~ Cond + PosAff_Z + pKM_Z + Time + (1|p|id),
    data = df_L,
    prior = c(
...
```
```{r}
fit_A <- brm(mvbind(ANS_Z, ANF_Z) ~ Cond*Time + PosAff_Z + pKM_Z + (1|p|id),
    data = df_L,
    prior = c(
      set_prior("normal(0,2)", class = "b", coef = "pKM_Z", resp = c("ANSZ", "ANFZ")),
      set_prior("normal(.10,1)", class = "b", coef = "PosAff_Z", resp = "ANSZ"),
      ...
      set_prior("normal(0,2)", class = "b", coef = "CondAmuse", resp = c("ANSZ","ANFZ")),
      set_prior("normal(0,2)", class = "b", coef = "CondKM", resp = c("ANSZ","ANFZ")),
      set_prior("normal(0,2)", class = "b", coef = "CondAmuse:Time2", resp = c("ANSZ","ANFZ")),
      set_prior("normal(0,2)", class = "b", coef = "CondKM:Time2", resp = c("ANSZ","ANFZ")),
      set_prior("normal(0,2)", class = "Intercept", resp = c("ANSZ", "ANFZ"))
    ),
    warmup = 5000, iter = 10000, chains = 4)
...
```{r}
mcmc_trace(posterior,pars = c("b_ANSZ_CondKM:Time2", "b_ANSZ_CondAmuse:Time2"),
           facet_args = list(ncol = 1))
```


```{r}
mcmc_trace(posterior,pars = c("b_ANFZ_CondKM:Time2", "b_ANFZ_CondAmuse:Time2"),
           facet_args = list(ncol = 1))
```
...
```{r}
mcmc_acf(posterior,pars = c("b_ANSZ_CondKM:Time2", "b_ANSZ_CondAmuse:Time2"))
```

```{r}
mcmc_acf(posterior,pars = c("b_ANFZ_CondKM:Time2", "b_ANFZ_CondAmuse:Time2"))
```
...
#### Fitting our model
```{r}
get_prior(mvbind(RNS_Z, RNF_Z) ~ Cond*Time + PosAff_Z + pKM_Z + (1|p|id),
    data = df_L)
```

```{r}
fit_R1 <- brm(mvbind(RNS_Z, RNF_Z) ~ Cond + PosAff_Z + pKM_Z + Time + (1|p|id),
    data = df_L,
    prior = c(
...
```{r}
fit_R <- brm(mvbind(RNS_Z, RNF_Z) ~ Cond*Time + PosAff_Z + pKM_Z + (1|p|id),
    data = df_L,
    prior = c(
      set_prior("normal(.09,.03)", class = "b", coef = "pKM_Z", resp = "RNSZ"),
      set_prior("normal(-.05,.02)", class = "b", coef = "pKM_Z", resp = "RNFZ"),
      ...
      set_prior("normal(0,2)", class = "b", coef = "CondAmuse", resp = c("RNSZ","RNFZ")),
      set_prior("normal(0,2)", class = "b", coef = "CondKM", resp = c("RNSZ","RNFZ")),
      set_prior("normal(0,2)", class = "b", coef = "CondAmuse:Time2", resp = c("RNSZ","RNFZ")),
      set_prior("normal(0,2)", class = "b", coef = "CondKM:Time2", resp = c("RNSZ","RNFZ")),
      set_prior("normal(0,2)", class = "Intercept", resp = c("RNSZ", "RNFZ"))
    ),
    warmup = 5000, iter = 10000, chains = 4)
...
```{r}
mcmc_trace(posterior,pars = c("b_RNSZ_CondKM:Time2", "b_RNSZ_CondAmuse:Time2"),
           facet_args = list(ncol = 1))
```

```{r}
mcmc_trace(posterior,pars = c("b_RNFZ_CondKM:Time2", "b_RNFZ_CondAmuse:Time2"),
           facet_args = list(ncol = 1))
```
...
```{r}
mcmc_acf(posterior,pars = c("b_RNSZ_CondKM:Time2", "b_RNSZ_CondAmuse:Time2"))
```

```{r}
mcmc_acf(posterior,pars = c("b_RNFZ_CondKM:Time2", "b_RNFZ_CondAmuse:Time2"))
```
...
```{r}
fit_C1 <- brm(mvbind(CNS_Z, CNF_Z) ~ Cond + PosAff_Z + pKM_Z + Time + (1|p|id),
    data = df_L,
    prior = c(
...
```{r}
fit_C <- brm(mvbind(CNS_Z, CNF_Z) ~ Cond*Time + PosAff_Z + pKM_Z + (1|p|id),
    data = df_L,
    prior = c(
      set_prior("normal(0,2)", class = "b", coef = "pKM_Z", resp = "CNSZ"),
      set_prior("normal(0,2)", class = "b", coef = "pKM_Z", resp = "CNFZ"),
      ...
      set_prior("normal(0,2)", class = "b", coef = "CondAmuse", resp = c("CNSZ","CNFZ")),
      set_prior("normal(0,2)", class = "b", coef = "CondKM", resp = c("CNSZ","CNFZ")),
      set_prior("normal(0,2)", class = "b", coef = "CondAmuse:Time2", resp = c("CNSZ","CNFZ")),
      set_prior("normal(0,2)", class = "b", coef = "CondKM:Time2", resp = c("CNSZ","CNFZ")),
      set_prior("normal(0,2)", class = "Intercept", resp = c("CNSZ", "CNFZ"))
    ),
    warmup = 5000, iter = 10000, chains = 4)
...
```{r}
mcmc_trace(posterior,pars = c("b_CNSZ_CondKM:Time2", "b_CNSZ_CondAmuse:Time2"),
           facet_args = list(ncol = 1))
```

```{r}
mcmc_trace(posterior,pars = c("b_CNFZ_CondKM:Time2", "b_CNFZ_CondAmuse:Time2"),
           facet_args = list(ncol = 1))
```
...
```{r}
mcmc_acf(posterior,pars = c("b_CNSZ_CondKM:Time2", "b_CNSZ_CondAmuse:Time2"))
```

```{r}
mcmc_acf(posterior,pars = c("b_CNFZ_CondKM:Time2", "b_CNFZ_CondAmuse:Time2"))
```
...
What about a multilevel model version, with no priors?
```{r}
fit_RNS2 <- brm(RNS_Z ~ Cond + PosAff_Z + pKM_Z + Time + (1|id),
    data = df_L,
    warmup = 5000, iter = 10000, chains = 4)

describe_posterior(fit_RNS2)
```
Multilevel without priors seems far more like what I currently consider my final output. Effect estimates themselves are all about the same, but without priors (as done here) the CIs are wider. AGAIN Priors seem to do what they're supposed to do! (very cool)

I don't think this will change anything, but just using the |p| thing in the random intercept to check if it changes anything substantially.
```{r}
fit_RNS2 <- brm(RNS_Z ~ Cond + PosAff_Z + pKM_Z + Time + (1|p|id),
    data = df_L,
    warmup = 5000, iter = 10000, chains = 4)

describe_posterior(fit_RNS2)
```
Yeah, doesn't matter--just as expected (thank goodness)

Time as interaction to check (This actually seems like the better model I should re-do with)
```{r}
fit_RNS2 <- brm(RNS_Z ~ Cond*Time + PosAff_Z + pKM_Z + (1|id),
    data = df_L,
    warmup = 5000, iter = 10000, chains = 4)
...
```{r}
summary(lm(RNS_Z ~ Time, data = df_L))
```

Looks fine to me that time is character in the df, since time should be a categorical variable anyway, and operates the same as if it was a factor with 2 levels when put into a regression. I.e. the time variable shouldnt be introducing problems to the models as currently organized.
```{r}
as.factor(df_L$Time)
summary(lm(RNS_Z ~ as.factor(Time), data = df_L))
```

#### SUpplemental Analyses
Un-preregistered test of relatedness.

If PA "stacks" on the unique relatedness effects of pKM, we might see that happen directly from the conditions by themselves. If the case, we'd expect to see KM condition with larger effect than amusement on rel sat.
```{r}
fit_R2 <- brm(mvbind(RNS_Z, RNF_Z) ~ Cond*Time + (1|p|id),
    data = df_L,
    prior = c(
      set_prior("normal(0,2)", class = "b", coef = "CondAmuse", resp = c("RNSZ","RNFZ")),
      set_prior("normal(0,2)", class = "b", coef = "CondKM", resp = c("RNSZ","RNFZ")),
...
summary(fit_R2)
```
Ok, not that helpful since all of the effects have such wide CIs its rather suspect to make any firm conclusions tbh.

#### Interaction Only
I think this may be the better way to go actually. It doesn't conflate time like the direct effects of KM and PA do in the original models (I think thats what they do at least)


```{r}
fit_Ri <- brm(mvbind(RNS_Z, RNF_Z) ~ Cond*Time + (1|p|id),
    data = df_L,
    prior = c(
      set_prior("normal(0,2)", class = "b", coef = "CondAmuse", resp = c("RNSZ","RNFZ")),
      set_prior("normal(0,2)", class = "b", coef = "CondKM", resp = c("RNSZ","RNFZ")),
...
```
```{r}
describe_posterior(fit_Ri)
```
```{r}
lm(df$RNS_2_Z ~ df$RNS_1_Z + df$Cond + df$pKM_Z)
```


##### Autonomy Needs: Pivotting to Long (edit)
```{r}
ANS_L <- df %>% select(id, ANS_1_Z, ANS_2_Z) %>% 
  pivot_longer(
    cols = !id,

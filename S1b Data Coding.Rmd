...
```{r}
df <- select(df,
       -(StartDate:UserLanguage)) #removing participant metadata columns (unnecessary variables)
...
Numeric Data
```{r}
df[, c(3:70, 72:112)] <- sapply(df[, c(3:70, 72:112)], as.numeric)
...
df$RNS1 <- rowMeans(df[,c("Comp1S_1", "Comp1S_2", "Comp1S_3", "Comp1S_4")], na.rm = T)

#Comp Frus itmes: 5, 6, 7, 8
df$RNF1 <- rowMeans(df[,c("Comp1F_5", "Comp1F_6", "Comp1F_7", "Comp1F_8")], na.rm = T)
```
Empathic Concern
...
pKM
"Probability of KM", or p(KM), is the final implementation of the sigmoid function on this centered KM variable.
...
```{r}
#Comp Sat items: 1, 2, 3, 4
df$RNS2 <- rowMeans(df[,c("Comp2S_1", "Comp2S_2", "Comp2S_3", "Comp2S_4")], na.rm = T)

#Comp Frus itmes: 5, 6, 7, 8
df$RNF2 <- rowMeans(df[,c("Comp2F_5", "Comp2F_6", "Comp2F_7", "Comp2F_8")], na.rm = T)
```

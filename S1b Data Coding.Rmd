...
fit_M <- brm(mvbind(pKM_Z, PosAff_Z) ~ Cond,
    df,
    prior = c(
      set_prior("normal(1.4,.10)", class = "b", resp = "pKMZ"),
      set_prior("normal(0,2)", class = "b", resp = "PosAffZ"),
...
```
##### ANS #####
The conditions by themselves had little to no effect, so much so as to be effectively null, on the outcome. The pre-test score accounted for the vast majority of the variance in T2 autonomy (T1, beta = .80). PA seemed to have a positive impact on ANS residualized change, while KM by itself had null effect.

##### ANF #####
The conditions by themselves had little to no effect, so much so as to be effectively null, on the outcome. The pre-test score accounted for the vast majority of the variance in T2 autonomy (T1, beta = .87). PA seemed to have a negative impact on ANF residualized change, while KM by itself had null effect.


TLDR: KM, sans it's PA feature, does little to Autonomy needs.
...
```
##### RNS #####
The conditions by themselves had little to no effect, so much so as to be effectively null, on the outcome. The pre-test score accounted for the vast majority of the variance in T2 autonomy (T1, beta = .75). PA seemed to have a positive impact on RNS residualized change, while KM by itself had marginally non-null effects.


##### RNF #####
The conditions by themselves had little to no effect, so much so as to be effectively null, on the outcome. The pre-test score accounted for the vast majority of the variance in T2 autonomy (T1, beta = .86). PA seemed to have small to null negative impact on RNF residualized change, while KM by itself had marginally non-null effects.

TLDR: KM, sans PA feature, has little to null effects (but in logical direction) for relatedness needs sat and frust.

Competency
```{r}
describe_posterior(fit_CS)
describe_posterior(fit_CF)
```
##### CNS #####
The conditions by themselves had little to no effect, so much so as to be effectively null, on the outcome. The pre-test score accounted for the vast majority of the variance in T2 autonomy (T1, beta = .84). PA seemed to have a positive impact on CNS residualized change, while KM by itself had null effect.

##### CNF #####
The conditions by themselves had little to no effect, so much so as to be effectively null, on the outcome. The pre-test score accounted for the vast majority of the variance in T2 autonomy (T1, beta = .84). PA seemed to have a negative impact on CNF residualized change, while KM by itself had null effect.

TLDR: KM, sans PA feature, has null effect on competency needs.
